...
class GenAlg(object):
    ...
    def qbs(self):
        if not self._qbs:
            self._qbs = [player for player in self.players if player.position == 'QB' and player.points >= 10]
        return self._qbs

    @property
    def rbs(self):
        if not self._rbs:
            self._rbs = [player for player in self.players if player.position == 'RB' and player.points >= 5]
        return self._rbs

    @property
    def wrs(self):
        if not self._wrs:
            self._wrs = [player for player in self.players if player.position == 'WR' and player.points >=5]
        return self._wrs

    @property
    def tes(self):
        if not self._tes:
            self._tes = [player for player in self.players if player.position == 'TE' and player.points >= 5]
        return self._tes

    @property
    def dsts(self):
        if not self._dsts:
            self._dsts = [player for player in self.players if player.position == 'DST' and player.points >= 5]
        return self._dsts
    ...
    def run_solver(self, players, pop_size=25000, salary_cap=50000, points_floor=120):
        '''
    ...
    if os.path.exists(fn):
        ...
        for p in players:
            ...
            if names.get(n):
                ...
                names[n] = p
        ga.players = ga.load_players(players=names.values(), projection_formula='tournament')
