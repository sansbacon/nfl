...
class FFNerdNFLParser(NFLProjectionsParser):
    ...
    def projections (self, projections, rankings):
        '''
        ...
        for id, player in player_rankings.items():
            ...
            if float(player.get('overall_rank')) > self.overall_rank_max or player.get('position') not in ['QB', 'WR', 'RB', 'TE']:
                pass
