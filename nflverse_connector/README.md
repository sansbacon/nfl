...
[nflverse](https://github.com/nflverse/nfl_data_py) into Databricks.

## Prerequisites

- `nfl_data_py` installed on your pipeline cluster:
  Add `nfl_data_py` to your pipeline's **Libraries** configuration.
...
- `play_by_play` is intentionally excluded — it's large (
  ~50+ columns, thousands of rows per season) and is better handled
  as a direct `nfl_data_py` notebook pipeline.
