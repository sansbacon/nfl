...
from sportscraper.utility import flatten_list
...
class PlayerGameFinder(Cmd):
    ...
    def _dump_search(self, df, pos):
        ...
        df.drop_duplicates(
            subset=["player", "week_num", "team"], keep="first", inplace=True
        )

        try:
            dfr = df[cols].copy()
            dfr.rename(
                columns={"week_num": "week", "draftkings_points": "dkpts"}, inplace=True
            )
    ...
    def do_search(self, inp):
        if "|" in inp:
            params = dict(
                itertools.zip_longest(*[iter(inp.split("|"))] * 2, fillvalue="")
            )
        else:
            params = {
                "opp_id": self.opp,
                "pos[]": self.pos.upper(),
                "c2val": str(self.thresh),
            }
        if self.data:
...
if __name__ == "__main__":
    # TODO: need to be able to set season or season range
    # TODO: the search interface is overly complex
    # TODO: have default column display for various positions
