...
class Parser(object):
    ...
    def player_core(self, data):
        ...
        try:
            ...
            pass
        try:
            context['best_comparable'] = data['Core']['Best Comparable Player']['Player_ID']
        except:
            pass
    ...
    def player_game_logs(self, data):
        ...
        logs = []
        gl_mapping = {
            'Carries': 'carries', 'Completion Percentage': 'completion_percentage',
            'Evaded Tackles': 'evaded_tackles', 'Fantasy Points': 'fantasy_points',
            'Interceptions': 'interceptions', 'Opponent': 'opponent',
            'Opportunity Share': 'opportunity_share', 'Pass Attempts': 'pass_attempts',
            'Passing Touchdowns': 'passing_touchdowns', 'Passing Yards': 'passing_yards',
            ...
            'Red Zone Targets': 'red_zone_targets', 'Rushing Touchdowns': 'rushing_touchdowns',
            'Rushing Yards': 'rushing_yards', 'Snap Share': 'snap_share',
            'Target Share': 'target_share', 'Targets': 'targets',
            'Total Touchdowns': 'total_touchdowns', 'Total Yards': 'total_yards',
            'Yards Per Opportunity': 'yards_per_opportunity', 'Yards Per Target': 'yards_per_target'
        }
    ...
    def player_medical_history(self, data):
        ...
        for seas, seaslog in data['Medical History'].items():
            for injury in seaslog:
                context = {}
                player = {'season_year': seas, 'site': 'playerprofiler', 'site_player_id': data['Player_ID']}
                context.update(player)
                mh = {mh_mapping[k]: v for k, v in injury.items() if k in mh_mapping.keys()}
                context.update(mh)
    ...
    def player_performance_metrics(self, data):
        ...
        # Workout Metrics
        pm_mapping = {
            'ADP': 'adp', 'ADP Trend': 'adp_trend',
            'Air Yards': 'air_yards', 'Air Yards Per Attempt': 'air_yards_per_attempt',
            'Air Yards Per Attempt Rank': 'air_yards_per_attempt_rank',
            'Air Yards Rank': 'air_yards_rank',
            'Attempts Per Game': 'attempts_per_game',
            'Carries': 'carries', 'Carries Per Game': 'carries_per_game',
            'Carries Per Game Rank': 'carries_per_game_rank',
            'Carries Rank': 'carries_rank', 'Completion Percentage': 'completion_percentage',
            ...
            'Dropbacks': 'dropbacks', 'Fantasy Points Per Attempt': 'fantasy_points_per_attempt',
            ...
            'Games': 'games', 'Interceptions': 'interceptions',
            'Offensive Line': 'offensive_line', 'Offensive Line Rank': 'offensive_line_rank',
            'Pass Attempts Rank': 'pass_attempts_rank', 'Passer Rating': 'passer_rating',
            'Passer Rating Rank': 'passer_rating_rank', 'Passing Attempts': 'passing_attempts',
            'Passing Touchdowns': 'passing_touchdowns', 'Passing Touchdowns Rank': 'passing_touchdowns_rank',
            'Passing Yards': 'passing_yards', 'Passing Yards Per Attempt': 'passing_yards_per_attempt',
            'Passing Yards Per Attempt Rank': 'passing_yards_per_attempt_rank',
            'Passing Yards Per Game': 'passing_yards_per_game', 'Passing Yards Rank': 'passing_yards_rank',
            'Production Premium': 'production_premium', 'Production Premium Rank': 'production_premium_rank',
            'Receiver Efficiency': 'receiver_efficiency', 'Receiver Efficiency Rank': 'receiver_efficiency_rank',
            'Red Zone Attempts': 'red_zone_attempts', 'Red Zone Attempts Per Game': 'red_zone_attempts_per_game',
            'Red Zone Attempts Rank': 'red_zone_attempts_rank', 'Red Zone Carries': 'red_zone_carries',
            ...
            'Rush Yards Per Game': 'rush_yards_per_game', 'Rush Yards Per Game Rank': 'rush_yards_per_game_rank',
            'Rushing Touchdowns': 'rushing_touchdowns', 'Rushing Touchdowns Rank': 'rushing_touchdowns_rank',
            'Rushing Yards': 'rushing_yards', 'Rushing Yards Rank': 'rushing_yards_rank',
            'Sacks': 'sacks', 'Snap Share': 'snap_share', 'Snap Share Rank': 'snap_share_rank',
            'Team Pass Plays': 'team_pass_plays', 'Team Pass Plays Rank': 'team_pass_plays_rank',
            'Total Fantasy Points': 'total_fantasy_points', 'Total QBR': 'total_qbr',
            'Total QBR Rank': 'total_qbr_rank', 'Upcoming Schedule Strength': 'upcoming_schedule_strength',
            'Upcoming Schedule Strength Rank': 'upcoming_schedule_strength_rank',
            'VOS': 'vos', 'VOS Rank': 'vos_rank', 'Weekly Volatility': 'weekly_volatility',
            'Weekly Volatility Rank': 'weekly_volatility_rank'
        }
        pm = {pm_mapping[k]: v for k, v in data['Performance Metrics'].items() if k in pm_mapping.keys()}
        context.update(pm)
        return pm
    ...
    def player_workout_metrics(self, data):
        ...
        # Workout Metrics
        wm_mapping = {
            '20-Yard Shuttle': '20-yard_shuttle', '3-Cone Drill': '3-cone_drill',
            '3-Cone Drill Rank': '3-cone_drill_rank', '40-Yard Dash': '40-yard_dash',
            '40-Yard Dash Rank': '40-yard_dash_rank', 'Agility Score': 'agility_score',
            'Agility Score Rank': 'agility_score_rank', 'Athleticism Score': 'athleticism_score',
            'Athleticism Score Rank': 'athleticism_score_rank', 'Broad Jump': 'broad_jump',
            'Broad Jump Rank': 'broad_jump_rank', 'Burst Score': 'burst_score',
            'Burst Score Rank': 'burst_score_rank', 'SPARQ-x': 'sparq-x',
            'SPARQ-x Rank': 'sparq-x_rank', 'Speed Score': 'speed_score',
            'Speed Score Rank': 'speed_score_rank', 'Throw Velocity': 'throw_velocity',
            'Throw Velocity Rank': 'throw_velocity_rank', 'Vertical Jump': 'vertical_jump',
            'Vertical Jump Rank': 'vertical_jump_rank', 'Wonderlic Score': 'wonderlic_score',
            'Wonderlic Score Rank': 'wonderlic_score_rank'
        }
        wm = {wm_mapping[k]: v for k, v in data['Workout Metrics'].items() if k in wm_mapping.keys()}
        context.update(wm)
        return wm
