"""Unity Catalog persistence adapter for NFLverse datasets."""
...
class NflverseUCTableConfig:
    """NFLverse-specific UC table configuration."""
...
@dataclass(frozen=True, slots=True)
class NflverseUCVolumeConfig:
    """NFLverse-specific UC volume configuration."""

    catalog: str = "nfl"
    schema: str = "nv"
    volume: str = "nv_volume"
...
def persist_nflverse_to_uc_tables(
    frames: Mapping[str, pl.DataFrame],
    config: NflverseUCTableConfig | None = None,
    dry_run: bool = False,
) -> list[UCWriteResult]:
    """Write NFLverse DataFrames as UC Delta tables."""
    cfg = config or NflverseUCTableConfig()
...
def persist_nflverse_to_uc_volume(
    frames: Mapping[str, pl.DataFrame],
    config: NflverseUCVolumeConfig | None = None,
    dry_run: bool = False,
) -> list[UCWriteResult]:
    """Write NFLverse DataFrames as files to a UC Volume."""
    cfg = config or NflverseUCVolumeConfig()
